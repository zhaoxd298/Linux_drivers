...
static ssize_t  my_fifo_read(struct file *filp, char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (devp->wr > devp->rd) {
    	remain_data = devp->wr - devp->rd;
    } else {
    	remain_data = devp->buf_size - (devp->rd - devp->wr); 
    }
...
static int get_fifo_remain_space(struct fifo_dev *devp)
{
	...
	if (devp->wr >= devp->rd) {
		remain_space = devp->buf_size - (devp->wr - devp->rd);
	} else {
		remain_space = devp->rd - devp->wr;
	}
...
static ssize_t  my_fifo_write(struct file *filp, const char __user *buffer, size_t count, loff_t* ppos)
{
    ...
    if (devp->wr >= devp->rd) {
    	right_space = devp->buf_size - devp->wr;
    	...
    	if (0 == left_space) {
    		copy_from_user(devp->buf + devp->wr, buffer, write_size);
    		devp->wr += write_size;
    		if (devp->wr == devp->buf_size) {
    			devp->wr = 0;
    		}
    	} else {
    		copy_from_user(devp->buf + devp->wr, buffer, right_space);
